...
const reportPath = 'docs/hbce-bank-evidence-core-v1-bank-review-briefing-index-2026-09-24.md';
...
pass('HBCE_BANK_REVIEW_BRIEFING_INDEX_EXISTS');

assert.match(doc, /^# HBCE Bank — Evidence Core v1 Bank Review Briefing Index/m);
assert.match(doc, /Status: bank review briefing index, non-canonical, non-production, non-authorizing\./);
assert.match(doc, /Repository: hermeticum-bce-bank/);
assert.match(doc, /Canonical upstream repository: hermeticum-bce-platform/);
assert.match(doc, /Current bank checkpoint: 8689f41e1e59a02da5a1ca6e6279caed2389b96f/);
pass('HBCE_BANK_REVIEW_BRIEFING_INDEX_HEADER');

assert.match(doc, /single bank-facing navigation layer/);
assert.match(doc, /risk, compliance, audit, security, innovation, procurement and executive review/);
assert.match(doc, /review pack index, exported review bundle manifest, outreach cover note, recipient matrix, missing evidence register, evidence request tracker and review closure report/);
assert.match(doc, /does not create production approval, regulatory approval, legal certification/);
pass('HBCE_BANK_REVIEW_BRIEFING_INDEX_PURPOSE');

assert.match(doc, /bank_review_briefing_index: true/);
assert.match(doc, /single_bank_facing_navigation_layer: true/);
...
pass('HBCE_BANK_REVIEW_BRIEFING_INDEX_STATUS_MODEL');

assert.match(doc, /\| 1 \| README\.md \| repository-level reviewer orientation \|/);
assert.match(doc, /\| 2 \| docs\/hbce-bank-evidence-core-v1-bank-review-pack-index-2026-09-24\.md \| formal bank review pack entry point \|/);
assert.match(doc, /\| 8 \| docs\/hbce-bank-evidence-core-v1-bank-review-closure-report-2026-09-24\.md \| aggregate zero-closure baseline \|/);
pass('HBCE_BANK_REVIEW_BRIEFING_INDEX_ENTRY_ORDER');

assert.match(doc, /\| Risk \| Bank review pack index \| Missing evidence register \|/);
assert.match(doc, /\| Compliance \| Bank review pack index \| Evidence request tracker \|/);
assert.match(doc, /\| Audit \| Exported review bundle manifest \| Review closure report \|/);
assert.match(doc, /\| Executive review \| README reviewer index \| Bank review briefing index \|/);
pass('HBCE_BANK_REVIEW_BRIEFING_INDEX_REVIEWER_ROUTING');

assert.match(doc, /The current closure baseline is:/);
assert.match(doc, /source_tracker_items: 43/);
assert.match(doc, /open_items: 43/);
assert.match(doc, /received_items: 0/);
assert.match(doc, /reviewed_items: 0/);
assert.match(doc, /closed_items: 0/);
assert.match(doc, /No external bank evidence has been received/);
assert.match(doc, /No external bank evidence has been reviewed/);
assert.match(doc, /No external bank evidence has been closed/);
pass('HBCE_BANK_REVIEW_BRIEFING_INDEX_CLOSURE_BASELINE');

for (const file of Object.values(files)) {
  assert.match(doc, new RegExp(esc(file)));
}
pass('HBCE_BANK_REVIEW_BRIEFING_INDEX_FILES_REFERENCED');

for (const hash of Object.values(expected)) {
  assert.match(doc, new RegExp(hash));
}
pass('HBCE_BANK_REVIEW_BRIEFING_INDEX_HASHES_REFERENCED');
...
pass('HBCE_BANK_REVIEW_BRIEFING_INDEX_VALIDATION_SUMMARY');
...
pass('HBCE_BANK_REVIEW_BRIEFING_INDEX_UPSTREAM');
...
pass('HBCE_BANK_REVIEW_BRIEFING_INDEX_BOUNDARY');

assert.match(doc, /At this bank-review-briefing-index stage/);
...
pass('HBCE_BANK_REVIEW_BRIEFING_INDEX_DELIVERY_FORM');

assert.match(doc, /freeze the current bank review package as a shareable review bundle/);
assert.match(doc, /add a bank review handoff checklist/);
assert.match(doc, /controlled derived banking evidence layer/);
pass('HBCE_BANK_REVIEW_BRIEFING_INDEX_NEXT_STEP');

for (const file of Object.values(files)) {
  assert.ok(fs.existsSync(file), `${file} missing`);
}
pass('HBCE_BANK_REVIEW_BRIEFING_INDEX_LOCAL_FILES_EXIST');

for (const [key, file] of Object.entries(files)) {
  assert.strictEqual(sha256(file), expected[key], `${file} hash mismatch`);
}
pass('HBCE_BANK_REVIEW_BRIEFING_INDEX_ALL_HASHES_MATCH');

assert.ok(fs.existsSync('protocol'));
assert.ok(fs.existsSync('schemas'));
assert.ok(fs.existsSync('tests'));
assert.ok(fs.existsSync('docs'));
pass('HBCE_BANK_REVIEW_BRIEFING_INDEX_REVIEW_DIRECTORIES_EXIST');

assert.ok(!fs.existsSync('package.json'));
assert.ok(!fs.existsSync('app'));
assert.ok(!fs.existsSync('pages'));
assert.ok(!fs.existsSync('src'));
pass('HBCE_BANK_REVIEW_BRIEFING_INDEX_NO_APPLICATION_SCAFFOLD');

const reportHash = sha256(reportPath);
assert.strictEqual(reportHash.length, 64);
pass('HBCE_BANK_REVIEW_BRIEFING_INDEX_FILE_HASH_STABLE');
