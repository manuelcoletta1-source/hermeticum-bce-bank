...
const doc = fs.readFileSync(trackerPath, 'utf8');
pass('HBCE_BANK_EVIDENCE_REQUEST_TRACKER_EXISTS');

assert.match(doc, /^# HBCE Bank — Evidence Core v1 Bank Evidence Request Tracker/m);
assert.match(doc, /Status: bank evidence request tracker, non-canonical, non-production, non-authorizing\./);
assert.match(doc, /Repository: hermeticum-bce-bank/);
assert.match(doc, /Canonical upstream repository: hermeticum-bce-platform/);
assert.match(doc, /Current bank checkpoint: 4249e98a060fb93ffae6f50b086c11f3e2b6b5cd/);
pass('HBCE_BANK_EVIDENCE_REQUEST_TRACKER_HEADER');

assert.match(doc, /extends the Bank Missing Evidence Register into an operational evidence request tracker/);
assert.match(doc, /adds due_date, assignee, received_artifact, review_result and closure_status/);
assert.match(doc, /does not create production approval, regulatory approval, legal certification/);
pass('HBCE_BANK_EVIDENCE_REQUEST_TRACKER_PURPOSE');

assert.match(doc, /The tracker is review-oriented/);
assert.match(doc, /The tracker is non-production/);
assert.match(doc, /The tracker is non-authorizing/);
assert.match(doc, /not a deployment plan/);
assert.match(doc, /not a closure certificate/);
pass('HBCE_BANK_EVIDENCE_REQUEST_TRACKER_STATUS');
...
pass('HBCE_BANK_EVIDENCE_REQUEST_TRACKER_FIELDS');

for (const value of ['P1', 'P2', 'P3', 'OPEN', 'REQUESTED', 'RECEIVED', 'REVIEWED', 'DEFERRED', 'NOT_REQUIRED', 'PROVIDE', 'ROUTE', 'REJECT_AS_OUT_OF_SCOPE', 'MARK_NOT_REQUIRED']) {
  assert.match(doc, new RegExp(esc(value)));
}
assert.match(doc, /due_date: TBD/);
assert.match(doc, /assignee: UNASSIGNED/);
assert.match(doc, /received_artifact: NOT_RECEIVED/);
assert.match(doc, /review_result: NOT_REVIEWED/);
assert.match(doc, /closure_status: OPEN/);
pass('HBCE_BANK_EVIDENCE_REQUEST_TRACKER_ENUMS');

for (const file of Object.values(files)) {
  assert.match(doc, new RegExp(esc(file)));
}
pass('HBCE_BANK_EVIDENCE_REQUEST_TRACKER_FILES_REFERENCED');

for (const hash of Object.values(expected)) {
  assert.match(doc, new RegExp(hash));
}
pass('HBCE_BANK_EVIDENCE_REQUEST_TRACKER_HASHES_REFERENCED');
...
pass('HBCE_BANK_EVIDENCE_REQUEST_TRACKER_UPSTREAM');

const rows = doc.split('\n').filter((line) => /^\| ME-\d{3} \|/.test(line));
assert.strictEqual(rows.length, 43);
pass('HBCE_BANK_EVIDENCE_REQUEST_TRACKER_ITEM_COUNT');

const ids = rows.map((line) => line.match(/^\| (ME-\d{3}) \|/)[1]);
const expectedIds = Array.from({ length: 43 }, (_, i) => `ME-${String(i + 1).padStart(3, '0')}`);
assert.deepStrictEqual(ids, expectedIds);
pass('HBCE_BANK_EVIDENCE_REQUEST_TRACKER_ID_SEQUENCE');

for (const row of rows) {
  const fields = row.split('|').slice(1, -1).map((value) => value.trim());
  assert.strictEqual(fields.length, 13, row);
}
pass('HBCE_BANK_EVIDENCE_REQUEST_TRACKER_ROW_FIELD_COUNT');

for (const row of rows) {
  assert.ok(row.includes('| TBD |'), row);
  assert.ok(row.includes('| UNASSIGNED |'), row);
  assert.ok(row.includes('| NOT_RECEIVED |'), row);
  assert.ok(row.includes('| NOT_REVIEWED |'), row);
  assert.ok(row.includes('| OPEN |'), row);
  assert.ok(row.includes('non-production evidence request'), row);
}
pass('HBCE_BANK_EVIDENCE_REQUEST_TRACKER_ROW_DEFAULTS');
...
pass('HBCE_BANK_EVIDENCE_REQUEST_TRACKER_OWNER_GROUPS');
...
pass('HBCE_BANK_EVIDENCE_REQUEST_TRACKER_REVIEWER_GROUPS');

assert.ok(rows.some((row) => row.includes('| P1 |')));
assert.ok(rows.some((row) => row.includes('| P2 |')));
assert.ok(rows.some((row) => row.includes('| P3 |')));
pass('HBCE_BANK_EVIDENCE_REQUEST_TRACKER_PRIORITY_COVERAGE');

assert.ok(rows.some((row) => row.includes('| PROVIDE |')));
assert.ok(rows.some((row) => row.includes('| ROUTE |')));
assert.ok(rows.some((row) => row.includes('| DEFER |')));
pass('HBCE_BANK_EVIDENCE_REQUEST_TRACKER_DECISION_COVERAGE');

for (const value of ['IN_REVIEW', 'CLOSED_ACCEPTED', 'CLOSED_REJECTED', 'CLOSED_DEFERRED']) {
  assert.match(doc, new RegExp(esc(value)));
}
pass('HBCE_BANK_EVIDENCE_REQUEST_TRACKER_CLOSURE_STATUS_MODEL');

for (const value of ['formal business case', 'target banking use case', 'pilot scope', 'risk register', 'control mapping', 'data protection assessment', 'threat model', 'target architecture', 'technical pilot plan']) {
  assert.match(doc, new RegExp(esc(value)));
}
pass('HBCE_BANK_EVIDENCE_REQUEST_TRACKER_REQUEST_VALUES');
...
pass('HBCE_BANK_EVIDENCE_REQUEST_TRACKER_DELIVERY_FORM');
...
pass('HBCE_BANK_EVIDENCE_REQUEST_TRACKER_BOUNDARY');

assert.match(doc, /At this bank-evidence-request-tracker stage/);
...
pass('HBCE_BANK_EVIDENCE_REQUEST_TRACKER_REPOSITORY_CAPABILITY');

assert.match(doc, /bank review closure report/);
assert.match(doc, /remain open, which have received artifacts, which were reviewed and which were closed/);
assert.match(doc, /controlled derived banking evidence layer/);
pass('HBCE_BANK_EVIDENCE_REQUEST_TRACKER_NEXT_STEP');

for (const file of Object.values(files)) {
  assert.ok(fs.existsSync(file), `${file} missing`);
}
pass('HBCE_BANK_EVIDENCE_REQUEST_TRACKER_LOCAL_FILES_EXIST');

for (const [key, file] of Object.entries(files)) {
  assert.strictEqual(sha256(file), expected[key], `${file} hash mismatch`);
}
pass('HBCE_BANK_EVIDENCE_REQUEST_TRACKER_ALL_HASHES_MATCH');

assert.ok(fs.existsSync('protocol'));
assert.ok(fs.existsSync('schemas'));
assert.ok(fs.existsSync('tests'));
assert.ok(fs.existsSync('docs'));
pass('HBCE_BANK_EVIDENCE_REQUEST_TRACKER_REVIEW_DIRECTORIES_EXIST');

assert.ok(!fs.existsSync('package.json'));
assert.ok(!fs.existsSync('app'));
assert.ok(!fs.existsSync('pages'));
assert.ok(!fs.existsSync('src'));
pass('HBCE_BANK_EVIDENCE_REQUEST_TRACKER_NO_APPLICATION_SCAFFOLD');

const trackerHash = sha256(trackerPath);
assert.strictEqual(trackerHash.length, 64);
pass('HBCE_BANK_EVIDENCE_REQUEST_TRACKER_FILE_HASH_STABLE');
